...
public class GuideActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    List<TextView>numList;  //为了便于管理，将其放到一个集合当中。表示页码的集合
...
//        初始化ViewPager的页面资源
        ...
        adapter = new GuideAdapter(viewList); //创建适配器对象
        guideVp.setAdapter(adapter); // 设置适配器
    ...
    private void setVPListener() {
        guideVp.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            @Override // 可以知道哪个位置被选中了
            public void onPageSelected(int position) {
                for (int i = 0; i < numList.size(); i++) {
                    numList.get(i).setTextColor(Color.WHITE); // 先将所有的设置为白色
                }
                numList.get(position).setTextColor(Color.RED); // 选中位置的设置为红色
